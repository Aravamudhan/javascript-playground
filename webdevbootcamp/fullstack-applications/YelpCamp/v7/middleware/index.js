...
middlewares.isLoggedInPage = function isLoggedInPage(req, res, next) {
    ...
    req.flash("error","You are not logged in. You need to be loggedin to perform the operation!!");
    res.redirect("/login");
}
...
middlewares.setLoggedInUser = function setLoggedInUser(req, res, next) {
    ...
    // If there is a flash message that is set somewhere we make that available to the ejs here.
    // This middleware gets called for every route. Because of that if we set a flash message
    // in some route and redirect from that route, this middleware will make the flash message
    // available in the redirected route.
